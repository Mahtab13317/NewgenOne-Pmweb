...
function ForwardTemplateMapping(props) {
  ...
  useEffect(() => {
    ...
    /*  const arrayOfObjects = Object.values(checkedTempObj);
    setMapList(arrayOfObjects) */
  }, []);
      ...
      temp.FwdVarMapping = (
        ...
        if (
          ...
          return {
            ...
            templateVarType: prev?.FwdVarMapping
              ? el?.templateVarType
              : el?.type,
            minOccurs: el?.minOccurs,
            orderId: el?.orderId,
            templateVarName: prev?.FwdVarMapping
              ? el?.templateVarName
              : el?.name,
  ...
  //Added on 25/10/2023, bug_id:135746
  const getMapVal = (val, data, index) => {
    /*  getVarListByType(variableDefinition, data).forEach((el, i) => {
     
    }); */

    const filterData = getVarListByType(variableDefinition, data).filter(
      (d) => d.VariableName === val
    );
    if (filterData?.length > 0) {
      return val;
    } else {
      props?.setIsValidMap(false);
      return null;
    }
  };
        ...
        {schemaList?.map((d, index) => {
          return (
            <div className={styles.dataDiv}>
              ...
              >
                <label
                  htmlFor={`Current_Process_Variable_dropdown_${d.name}`}
                  style={{ display: "none" }}
                >
                  Current_Process_Variable_dropdown
                </label>
                <Select
                  className={
                    direction === RTL_DIRECTION
