...
function BasicDetails(props) {
  ...
  const handleChange = (con) => {
    // added on 29/10/23 for BugId 135413
    let decodedInitialVal = decode_utf8(
      localLoadedActivityPropertyData?.ActivityProperty?.actGenPropInfo
        ?.genPropInfo?.description
    );
    if (!decodedInitialVal.includes("<p>")) {
      let newVal = encode_utf8(`<p>${decodedInitialVal}</p>`);
      decodedInitialVal = decode_utf8(newVal);
    }
    // till here BugId 135413

    // condition added on 29/10/23 for BugId 135413
    if (decodedInitialVal !== decode_utf8(con)) {
  ...
  useEffect(() => {
    ...
    if (saveCancelStatus.SaveOnceClicked) {
      ...
      //Modified on 06/09/2023, bug_id:135583
      else if (
        !validateRegex(
          localLoadedActivityPropertyData?.ActivityProperty?.actGenPropInfo
            ?.genPropInfo?.cost,
        ...
        localLoadedActivityPropertyData?.ActivityProperty?.actGenPropInfo?.genPropInfo?.cost.includes(
          "e"
        )
      ) {
        setShowCostError({
  ...
  const changeBasicDetails = (e, customName) => {
    ...
    //Modified on 06/09/2023, bug_id:135583
    else if (e.target.name === "cost" && e.target.value.length > 13) {
      dispatch(
        setActivityPropertyChange({
          [propertiesLabel.basicDetails]: { isModified: true, hasError: true },
        })
  ...
  const handleSwimlaneQueueCheck = () => {
    ...
    // added on 19/10/23 for BugId 139909
    temp?.Lanes?.map((el) => {
      if (+el.QueueId === +tempId) {
        isLaneQueue = true;
      }
    });
    // condition added on 19/10/23 for BugId 139909
    if (!isLaneQueue) {
      axios
  ...
  const isTablet = () => {
    ...
    return tabletKeywords.some((item) => userAgent.indexOf(item) !== -1);
  };
  return (
    <div className="flexScreen basicDetails-mainDiv">
      ...
      {showQueueModal ? (
        <Modal
          ...
          // commented on 19/10/23 for BugId 116205
          // modalClosed={() => setShowQueueModal(false)}
          // commented on 19/10/23 for BugId 139909
