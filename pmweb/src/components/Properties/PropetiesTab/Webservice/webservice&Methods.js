  ...
  tableContainer: {
    ...
    height: (props) =>
      props.isDrawerExpanded
        ...
        : `calc((${props.windowInnerHeight}px - ${headerHeight}) - 28rem)`,
    // height: "auto",
    // overflowX: 'hidden',
    // added code on 21-09-2023 for bugId: 135961
    "&:hover": {
...
export default function CustomizedTables(props) {
  ...
  return (
    // code added in table container on 23-20-23 for bug 139863
    <TableContainer component={Paper} className={`${classes.tableContainer} customScroller`} id="TableContainer">
        ...
        <TableHead>
          <StyledTableRow className={classes.tableRow}>
            <StyledTableCell
              ...
              style={{ width: "30vw", textAlign: "start" }}
            >
              {t("webService")}
            </StyledTableCell>
            <StyledTableCell
              className={classes.tableHeader}
              style={{ width: "65vw", textAlign: "start" }}
            >
              {t("method")}
            </StyledTableCell>
            <StyledTableCell
        ...
        <TableBody className="associatedTemplateDiv">
          {props.associations.map((row, index) => (
            <StyledTableRow key={row.webservice} className={classes.tableRow}>
              ...
              <StyledTableCell style={{ width: "2.5vw", textAlign: "start" }}>
                {
                  //!props.isReadOnly && ( //Modified on 02/11/2023, bug_id:140266 
                  <IconButton
                    onClick={() => mappingHandler(row)}
                    id={`pmweb_webService_WebserviceMethods_swap_${index}`}
                    // tabIndex={0}
                    onKeyUp={(e) => {
                      if (e.key === "Enter") {
                        mappingHandler(row);
                  ...
                  >
                    {/* Added Code for Bug 135904 */}
                    <Tooltip
                      title={t("ConfigureDataMapping")}
                      placement="bottom"
                    >
                      <SwapHorizIcon
                        style={{
               ...
               // ) //Modified on 02/11/2023, bug_id:140266 
                }
              </StyledTableCell>
              <StyledTableCell style={{ width: "2.5vw", textAlign: "start" }}>
                {!props.isReadOnly && (
                  <IconButton
                    ...
                    id={`pmweb_webService_WebserviceMethods_deleteIcon_${index}`}
                    // tabIndex={0}
                    onKeyUp={(e) => {
                      if (e.key === "Enter") {
                  ...
                  >
                    {/* Added Code for Bug 135904 */}
                    <Tooltip title={t("DeAssociate")} placement="bottom-start">
                      <DeleteIcon
                        style={{
                          width: "1.5rem",
                          height: "1.5rem",
