...
import { deleteDocument } from "./DocumentsApiCalls";
...
export default function Documents(props) {
  ...
  const deleteRow = async (previewId, rowIdToDelete) => {
    ...
    const isDeleted = await deleteDocument({
      previewId: previewId,
      docId: rowIdToDelete,
    });
    if (!isDeleted) {
              ...
              >
                ...
                >
                  ...
                  <Grid item>
                    ...
                    >
                      <RemoveIcon
                        data-testid={`Documents_deleteIcon_${row?.id}`}
                        style={{
                          cursor: "pointer",
                          width: "1.25rem",
                          height: "1.25rem",
    ...
    <Grid container className={classes.NoDataGrid}>
      ...
      <Grid item>
        <Typography
          ...
          data-testid="pmweb_CreateAI_Documents_NoDocuments"
        >
          {t("NoDocuments")}
        </Typography>
      </Grid>
      <Grid item style={{ display: "flex", justifyContent: "center" }}>
        <Typography
          ...
          data-testid="pmweb_CreateAI_Documents_NoDocumentsRegenrateMsg"
        >
          {t("NoDocumentsRegenerateMsg")}
        </Typography>
      </Grid>
    </Grid>
  );
