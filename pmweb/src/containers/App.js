...
const App = (props) => {
  ...
  useEffect(() => {
    if (brandDetails?.favIcon) {
      const favicon = document.querySelector('link[rel="icon"]');
      const customFavicon = brandDetails?.favIcon;
      favicon.href = customFavicon;
      const favicon2 = document.querySelector('link[rel="apple-touch-icon"]');

      const customFavicon2 = brandDetails?.favIcon;
      favicon2.href = customFavicon2;
    }
  }, [brandDetails]);
  useEffect(() => {
  ...
  axios.interceptors.response.use(
    ...
    async function (error) {
      // This condition is for handle for response type=blob, kindly dont remove this condition
      if (
        ...
        const errorMsg = JSON.parse(responseObj)?.error?.message;
        ...
        if (axios.isCancel(error)) {
          ...
          dispatch(
            ...
            //Modified  on 02/01/2024, bug_id:142037
            ...
            */
            setToastDataFunc({
              message: error?.response?.data?.error?.errors
                ? error?.response?.data?.error?.errors?.importFile
                : error?.response?.data?.errorMsg ||
                  error?.response?.data?.error?.message ||
                  error?.response?.data?.error,
